...
//
// Um has_car zu einer bestehenden Tabelle hinzuzufügen:
//   ALTER TABLE events ADD COLUMN has_car TINYINT(1) NOT NULL DEFAULT 0 AFTER is_all_day;
//
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $hasCar     = isset($_POST['has_car'])        ? (bool)$_POST['has_car']    : false;
        ...
        $stmt = $conn->prepare(
            'INSERT INTO events
                 (employer_id, user_id, date, date_to, start_time, end_time,
                  category_id, is_all_day, has_car, title)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
        );
        $stmt->bind_param(
            'iisssssiis',
...
$stmt = $conn->prepare(
    'SELECT e.id, e.employer_id, e.user_id,
            ...
            e.category_id, e.is_all_day, e.has_car, e.title,
