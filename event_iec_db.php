...
// ============================================================
// Alle Session-Einträge (Anwesenheitszeiten) für ein bestimmtes Datum abrufen.
//
// Die Datenbank speichert Login- und Logout-Ereignisse als separate Zeilen in
// der Tabelle a_tab_session.  Diese Funktion liest beide Ereignistypen ein und
// baut daraus zusammengesetzte Session-Objekte (login_time + logout_time).
//
...
//   ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
//
// Paarungsregel: Alle Login-Einträge eines Nutzers werden chronologisch mit
// den Logout-Einträgen desselben Nutzers gepaart (login[0]↔logout[0],
// login[1]↔logout[1], …).  Hat ein Login-Eintrag keinen passenden Logout,
// ist logout_time leer → Session gilt als aktiv.
//
// @param  string $date  Datum im Format Y-m-d (z. B. '2026-04-29')
// @return array         Array mit Session-Datensätzen:
//                         id           – ID des Login-Eintrags
//                         employer_id  – user_id des Mitarbeiters
//                         date         – Datum (Y-m-d)
//                         login_time   – Uhrzeit des Logins (HH:MM)
//                         logout_time  – Uhrzeit des Logouts (HH:MM) oder '' wenn noch aktiv
// ============================================================
function dbGetSessions(string $date): array
{
    $conn = getDbConnection();
    ...
    $logins  = [];   // $logins[$user_id]  = [[session_id, session_time], ...]
    $logouts = [];   // $logouts[$user_id] = [session_time, ...]
